...
const CommunityDetail: NextPage = ({ initialInput, ...props }: T) => {
	...
	/** LIFECYCLES **/
	useEffect(() => {
		if (articleId) setSearchFilter({ ...searchFilter, page: 1, search: { commentRefId: articleId } });
	}, [articleId]);
	...
	const updateButtonHandler = async (commentId: string, commentStatus?: CommentStatus.DELETE) => {
		try {
			...
			if (!commentStatus && updatedComment === comments?.find((comment) => comment?._id === commentId)?.commentContent) return;
